...
*Phase 1: 
Initial TriageNetwork Analysis (Wireshark)The RecordUser.pcapng file immediately provided two critical artifacts:
An HTTP Download: The executable downloads a small file named anonymous. This file was extracted and later identified as bytecode for the VM.
A TCP C2 Stream: The executable connects to 192.168.134.132 on port 8888 and sends a payload. This payload was extracted and identified as the encrypted second-stage DLL.Static Analysis (IDA Pro / Ghidra)
...
Key Generation: The program calculates the SHA256 hash of the string hackingisnotacrime. This 32-byte hash is used as the AES key.
Encryption: The program encrypts the file C:\ProgramData\Important\user.html using AES-256-ECB with the generated key.
Exfiltration: It then connects to the C2 server and sends the encrypted file.
...
IDA Pro / Ghidra: For static analysis and reverse engineering.
Python 3: For decryption and VM emulation.pycryptodome library for AES decryption.
